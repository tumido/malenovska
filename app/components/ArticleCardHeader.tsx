...
const getThumbnailUrl = (image: string, height?: number): string => {
  try {
    ...
    url.pathname =
      url.pathname.slice(0, delim) + suffix + url.pathname.slice(delim);
    return url.toString();
  } catch {
    return image;
  }
...
export const ArticleCardHeader = ({
  ...
  const titleClass =
    titleVariant === "small" ? "text-xl" : "text-2xl md:text-3xl";
    ...
    <div className="relative" style={{ height }}>
      ...
      {title && (
        <div className="absolute bottom-0 w-full bg-black/30 px-4 py-3 pt-4 text-white backdrop-blur-sm">
          <h2 className={`font-display ${titleClass}`}>{title}</h2>
        </div>
      )}
    </div>
  );
