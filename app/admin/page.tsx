...
const DashboardPage = () => {
  ...
  const quickActions = [
    { href: "/admin/events/new", label: "Nová událost", icon: Plus },
    { href: `/admin/legends/new${eventId ? `?event=${eventId}` : ""}`, label: "Nová legenda", icon: ScrollText },
    { href: `/admin/races/new${eventId ? `?event=${eventId}` : ""}`, label: "Nová strana", icon: Users },
    { href: `/admin/galleries/new${eventId ? `?event=${eventId}` : ""}`, label: "Nová galerie", icon: Image },
    ...(eventId ? [{ href: `/admin/participants?event=${eventId}`, label: "Účastníci", icon: UserCheck }] : []),
  ];
