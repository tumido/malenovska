  ...
  if (!event.registrationAvailable && !isDev) {
    return (
      <div className="flex min-h-[50vh] items-center justify-center">
        <p className="text-grey-400 text-lg">Registrace není dostupná.</p>
      </div>
    );
  }
        ...
        <div className="flex flex-col items-center gap-6 py-12">
          ...
          {result && (
            <div className="flex flex-wrap justify-center gap-3">
              <button
                onClick={handleReset}
                className="rounded bg-primary px-6 py-3 text-white shadow hover:bg-white/10"
              >
                Nová registrace
              </button>
              <Link
              ...
              {parseInt(formData.age, 10) < 18 && event.declaration && (
                <a
                  ...
                  target="_blank"
                  rel="external"
                  className="rounded bg-secondary px-6 py-3 text-white shadow hover:bg-secondary-dark"
                >
          ...
          {result?.variant === "error" && (
            <div className="mx-auto max-w-md rounded border border-red-500/30 bg-red-500/10 p-4 text-red-300">
              Registrace nebyla provedena: {result.message}
            </div>
          )}
          {parseInt(formData.age, 10) < 18 && (
            <div className="mx-auto max-w-md rounded border border-yellow-500/30 bg-yellow-500/10 p-4 text-yellow-300">
              Ještě ti nebylo 18 let. Nezapomeň si stáhnout, vyplnit a hlavně
              přinést podepsané potvrzení pro nezletilé.
            </div>
          )}
        </div>
        </div>
  ...
  return (
    <>
      ...
      <Article>
        <div className="hidden sm:flex items-center justify-center gap-2 p-5">
          {stepNames.map((name, i) => (
            <div key={i} className="flex items-center gap-2">
              <span
                className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-bold ${i <= step ? "bg-secondary text-white" : "bg-grey-400/20 text-grey-500"}`}
              >
                {i + 1}
              </span>
              <span className={i <= step ? "text-primary" : "text-grey-500"}>
                {name}
              </span>
              {i < stepNames.length - 1 && (
                <span className="mx-2 h-px w-8 bg-grey-400" />
              )}
            </div>
          ))}
        </div>
        ...
        {isLoading ? (
          <div className="p-6">
            <div className="h-75 animate-pulse rounded bg-grey-500/20" />
          </div>
        ) : (
          <>
            {step === 0 && (
              <div className="p-6">
                  ...
                  {races!.map((race) => {
                    ...
                    return (
                      <SmallArticleCard
                        ...
                        image={race.image}
                        selected={formData.race === race.id}
                        disabled={count >= race.limit}
                ...
                {errors.race && (
                  <p className="mt-2 text-sm text-red-400">{errors.race}</p>
                )}
                ...
                <ArticleCardHeader
                  ...
                  title={
                    <span className="flex items-center gap-2">
                      {selectedRace.name}
                      <span className="rounded-full bg-primary px-2 py-0.5 text-sm">
            ...
            {step === 2 && selectedRace && (
              <div className="p-6">
                ...
                <hr className="my-6 border-grey-400/30" />
                ...
                <div className="mt-4">
                  <Input label="Strana" value={selectedRace.name} readOnly />
                </div>
                <hr className="my-6 border-grey-400/30" />
                    ...
                    return (
                      <div key={i} className="mb-4">
                        <Input
                          ...
                          value={String(formData[extra.props.id ?? ""] ?? "")}
                          onChange={(v) =>
        ...
        <div className="flex justify-center gap-4 p-5">
          <button
            ...
            disabled={step === 0}
            className="rounded px-6 py-2 text-primary disabled:opacity-30 hover:bg-black/5"
          >
            Zpět
          </button>
          {step < 2 ? (
            <button
              onClick={handleNext}
              className="rounded bg-secondary px-6 py-2 text-white hover:bg-secondary-dark"
            >
              Další
            </button>
          ) : (
