...
const InfoPage = () => {
  ...
  const timelineItems = [
    {
      label: "Začátek akce",
      time: event.onsiteStart ? toTimeStr(event.onsiteStart) : undefined,
    },
    {
      label: "Otevření registrace",
      time: event.onsiteRegistrationOpen
        ? toTimeStr(event.onsiteRegistrationOpen)
        : undefined,
    },
    {
      label: "Uzavření registrace",
      time: event.onsiteRegistrationClose
        ? toTimeStr(event.onsiteRegistrationClose)
        : undefined,
    },
    {
      label: "Konec akce",
      time: event.onsiteEnd ? toTimeStr(event.onsiteEnd) : undefined,
    },
              ...
              <div className="flex items-center gap-4 px-4 py-3">
                <div className="relative flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-secondary/10">
                  ...
                  {timelineItems.length > 0 && (
                    <div className="absolute top-full h-9.25 w-px bg-primary/20" />
                  )}
                </div>
                <div>
                    ...
                    {timelineItems.map((item, i) => (
                      <div key={i} className="flex items-baseline">
                        <div className="relative flex w-3 shrink-0 items-center justify-center">
                          ...
                          <div
                            className={`relative z-10 h-2.5 w-2.5 rounded-full ${i === 0 ? "bg-secondary" : "border-2 border-secondary bg-primary-light"}`}
                          />
                        </div>
                        <span className="ml-7.5 w-12 shrink-0 font-display text-sm leading-none font-bold text-primary">
                          {item.time}
                        </span>
                        <span className="ml-3 text-sm leading-none text-primary/60">
                          {item.label}
                        </span>
                      </div>
                    ))}
                ...
                <div className="pointer-events-auto flex flex-wrap justify-center gap-2">
                  {event.poi.map((poi, index) => (
                    <button
                      key={index}
                      onClick={() =>
                        handlePoiClick(index, poi.latitude, poi.longitude)
                      }
                    ...
                    >
                      <span
                        ...
                        style={
                          poi.color ? { backgroundColor: poi.color } : undefined
                        }
                      />
                      {poi.name}
                      <a
                ...
                {activePoi !== null && event.poi[activePoi]?.description && (
                  <div className="pointer-events-auto rounded-full bg-primary/80 px-4 py-1.5 text-sm text-grey-300 shadow-md backdrop-blur-sm">
                    <MapPin
                      ...
                      style={{ color: event.poi[activePoi].color }}
                    />
